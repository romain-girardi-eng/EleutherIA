...
- ``shapes/quality/`` — quality goals that surface a triage backlog but
  ...
  (id prefix conventions), ``Period`` (canonical period whitelist), and
  ``DescriptionHygiene`` (no markdown in non-passage descriptions).
...
- ``scripts/audit_kg_quality.py`` — CLAIM_TYPES, EVIDENCE_RELATIONS,
  CANONICAL_PERIODS (re-imported here to keep shapes in lockstep with the
  Python audit)
"""
...
# Update both places together if anything shifts.
...
CANONICAL_PERIODS: Final[frozenset[str]] = frozenset(
    {
        "Presocratic",
        "Classical Greek",
        "Hellenistic",
        ...
        "Modern",
        "Contemporary",
        # ``Cross-period`` tags philosophical positions (fatalism,
        # libertarianism, compatibilism) that span multiple periods. It is
        # a meaningful semantic category, not a placeholder.
        "Cross-period",
    }
)
