 ...
 *
 * Shows a friendly, informative message when the backend is experiencing delays.
 * Handles cold starts and other potential latency issues gracefully.
 *
 * Features:
 * - Only shows after 3 seconds (avoids flashing for fast requests)
 * - Progress animation to keep user informed
 * - Educational about potential delays
...
export const ColdStartLoader = ({
  ...
  const getProgressMessage = () => {
    if (elapsedSeconds < 10) {
      ...
      return 'Still initializing resources...';
    } else if (elapsedSeconds < 60) {
      return 'Almost there! The server is warming up...';
    } else {
      ...
      <div className="max-w-md w-full bg-white border-2 border-blue-200 rounded-lg shadow-lg p-6 space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
          <p className="text-xs text-gray-700 leading-relaxed">
            <strong>Why is this happening?</strong> The backend may be experiencing higher than usual load
            or initializing resources. This delay is temporary, and subsequent requests will be fast!
          </p>
        </div>
...
export const ColdStartLoaderMinimal = ({ isLoading }: { isLoading: boolean }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-6">
      <AILoader text="Connecting" size="md" />
      {showMessage && (
        <div className="text-center space-y-2 max-w-md px-4 mt-4">
