...
export default function GraphLegend({ className, types }: GraphLegendProps) {
  const legendTypes = (types && types.length > 0 ? types : DEFAULT_TYPES).slice(0, 6);

  return (
    <div
      className={cn(
        'flex flex-wrap items-center gap-2 rounded-2xl border border-stone-200/80 bg-white/82 px-3 py-2.5 shadow-[0_16px_36px_-28px_rgba(120,53,15,0.3)] backdrop-blur-xl',
        className,
      )}
    >
