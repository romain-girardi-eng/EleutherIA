    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-900">{t('graphrag.smartSuggestions.title')}</h3>
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          >
            {t('graphrag.smartSuggestions.allLabel')} ({filteredSuggestions.length})
          </button>
          ...
          {Object.entries(SUGGESTION_CATEGORIES).map(([cat, config]) => {
            ...
            return (
              ...
              >
                ...
                <span className="capitalize hidden sm:inline">{t(`graphrag.smartSuggestions.${cat}`)}</span>
      ...
      <AnimatePresence mode="popLayout">
        {displayedSuggestions.length === 0 ? (
          ...
          >
            ...
            <p className="text-sm">{t('graphrag.smartSuggestions.noSuggestions')}</p>
          </motion.div>
        ) : (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full border flex-shrink-0 ${getComplexityBadge(suggestion.estimatedComplexity)}`}>
                      {t(`graphrag.smartSuggestions.${suggestion.estimatedComplexity}`)}
                    </span>
                  </div>
      ...
      <div className="mt-3 p-2 bg-slate-50 border border-slate-200 rounded-lg">
        <p className="text-xs text-gray-600">
          <span className="font-semibold">💡 {t('graphrag.quality.tip')}</span> {t('graphrag.quality.tipText')}
        </p>
      </div>
    </div>
  );
