...
export default function PathInspectorPanel() {
  ...
  const pathLength = result?.nodes?.length || 0;

  return (
    <AccordionPanel
      title="Path Inspector"
      icon={<Route className="w-5 h-5" />}
      badge={pathLength > 0 ? `${pathLength} hops` : undefined}
      defaultExpanded={false}
    >
      <div className="text-xs text-academic-muted mb-4">Resolve narrative walks between any two entities</div>
