...
function InfluenceMatrixPanelComponent() {
  ...
  const totalMappedEdges = influenceMatrix?.totals?.edgesMapped || 0;
  const totalSchools = influenceMatrix?.totals?.schoolsCovered || 0;
  ...
  return (
    <AccordionPanel
      title="Influence Matrix"
      icon={<Grid3x3 className="w-5 h-5" />}
      badge={`${totalSchools} schools, ${totalMappedEdges} edges`}
      defaultExpanded={false}
    >
