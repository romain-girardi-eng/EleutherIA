  ...
  return (
    <div className="w-full">
      <div
        ...
        className={cn(
          'relative flex flex-col items-center justify-center gap-4 rounded-2xl px-6 py-10 sm:px-8 sm:py-16 text-center transition-all',
          'cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2',
          // On touch devices, drop the dashed dropzone look — show a clear,
          // solid-bordered card with a prominent "Choose a PDF" affordance.
          isTouch
            ? 'border border-amber-300 bg-amber-50/40 active:bg-amber-50/70'
      ...
      >
        <div className="flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-amber-100 text-amber-600">
          <Upload className="h-7 w-7 sm:h-8 sm:w-8" aria-hidden="true" />
        </div>
        <div>
          <p className="text-base sm:text-lg font-semibold text-stone-800">
            {isTouch
              ? t('contribute.upload.chooseTouch', 'Choisir un PDF')
        ...
        {isTouch && (
          <span className="inline-flex min-h-[44px] items-center justify-center rounded-full bg-amber-600 px-5 py-2 text-sm font-semibold text-white shadow-sm">
            {t('contribute.upload.chooseTouchButton', 'Parcourir')}
          </span>
        )}
