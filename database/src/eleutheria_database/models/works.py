...
class AncientWork(BaseModel):
    ...
    work_id: UUID
    canonical_id: str = Field(..., description="Unique identifier, e.g., 'chrysippus_on_fate'")
    title: str
    title_original: str | None = Field(None, description="Original Greek/Latin title")
    author: str
    author_original: str | None = Field(None, description="Original Greek/Latin author name")
    language: str = Field(..., description="Language code: grc, lat, eng, hbo, ara")
    period: str | None = Field(None, description="Historical period, e.g., 'Hellenistic'")
    date_composed: str | None = Field(None, description="Approximate date, e.g., '3rd c. BCE'")
    ...
    division_scheme: str | None = Field(None, description="e.g., 'book.chapter.section'")
    ...
    citation_levels: list[str] | None = Field(None, description="e.g., ['book', 'chapter', 'verse']")
...
class Passage(BaseModel):
    ...
    passage_id: UUID
    work_id: UUID
    canonical_ref: str = Field(..., description="Reference, e.g., '3.191' or 'Matthew 5:3'")
...
class PassageCitation(BaseModel):
    ...
    citation_type: str | None = Field(None, description="e.g., 'primary_source', 'secondary_source'")
...
class PassageRelationship(BaseModel):
    ...
    relationship_type: str = Field(..., description="e.g., 'quotes', 'alludes_to', 'parallel'")
