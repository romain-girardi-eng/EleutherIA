#!/usr/bin/env python3
"""Gate and optionally apply the 2026-08-17 reading-ingestion wave C.

Idempotently merges ``scripts/data_2026_08_17_reading_wave_c.json`` into
``data/kg/{nodes,edges}.jsonl`` and promotes existing Origenality shells:
...
- when this wave adds ``opposes`` edges, ``--apply`` also requires the G6
  reachability pin to equal the exact post-apply count;
- backups use suffix ``.bak-reading_c``.

Usage: python3 scripts/ingest_2026_08_17_reading_wave_c.py [--apply]
Default is dry-run.
"""
...
DELTA = ROOT / "scripts/data_2026_08_17_reading_wave_c.json"
GATE = ROOT / "scripts/check_ingestion_rules.py"
G6_PROBE = ROOT / "graphrag/tests/g6/test_reachability_probe.py"
BAK_SUFFIX = ".bak-reading_c"
INGEST_SCRIPT = "scripts/ingest_2026_08_17_reading_wave_c.py"
PROMOTION_STAMP = "reading_wave_c_2026_08_17"
...
def run_ingestion_gate(
    ...
    with tempfile.TemporaryDirectory(prefix="reading-wave-c-gate-") as tmp_name:
        ...
        subset = sandbox / "reading_wave_c_novel.json"
        subset.write_text(
            json.dumps(
...
def main() -> int:
    ...
    for original_target, updates in ready_enrichments:
        ...
        md.update(updates)
        md["reading_c_promotion_stamp"] = PROMOTION_STAMP
