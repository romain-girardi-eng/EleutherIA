...
def find_orphaned_edges(
    edges: list[dict[str, Any]], node_ids: set[Any]
) -> list[dict[str, Any]]:
...
def analyze_bidirectional_consistency(
    edges: list[dict[str, Any]], nodes_by_id: dict[Any, dict[str, Any]]
) -> list[dict[str, Any]]:
...
def calculate_quality_score(
    ...
    scores = {
        "orphan_penalty": max(0, 100 - (len(orphaned) / total_edges * 100))
        if total_edges > 0
        else 0,
        "metadata_completeness": (1 - len(missing_metadata) / total_edges * 1) * 100
        if total_edges > 0
        else 0,
        "node_coverage": len(
    ...
    try:
        ...
        if orphaned:
            ...
            for i, orph in enumerate(orphaned[:20], 1):  # Show first 20
                ...
                print(
                    f"   {orph['source_id']} --[{orph['relation']}]--> {orph['target_id']}"
                )
            if len(orphaned) > 20:
        ...
        print(
            f"Edges with metadata: {metadata_stats['with_metadata']} ({metadata_stats['with_metadata'] / edge_count * 100:.1f}%)"
        )
        print(
            f"Edges without metadata: {metadata_stats['without_metadata']} ({metadata_stats['without_metadata'] / edge_count * 100:.1f}%)"
        )
        print()
        ...
        if metadata_stats["metadata_fields"]:
            ...
            for field, count in sorted(
                metadata_stats["metadata_fields"].items(),
                key=lambda x: x[1],
                reverse=True,
            ):
        ...
        if missing_reciprocal:
            print(
                f"⚠️  Found {len(missing_reciprocal)} edges missing reciprocal relationships"
            )
            print()
            for i, miss in enumerate(missing_reciprocal[:15], 1):
                print(
                    f"{i}. {miss['source_name']} --[{miss['existing_relation']}]--> {miss['target_name']}"
                )
                print(
                    f"   Missing: {miss['target_name']} --[{miss['missing_relation']}]--> {miss['source_name']}"
                )
        ...
        if temporal_violations:
            print(
                f"⚠️  Found {len(temporal_violations)} potential temporal inconsistencies"
            )
            print()
            for i, viol in enumerate(temporal_violations[:10], 1):
                print(
                    f"{i}. {viol['source']} --[{viol['relation']}]--> {viol['target']}"
                )
                print(f"   {viol['issue']}")
        ...
        print(
            f"  Metadata completeness: {score_breakdown['metadata_completeness']:.1f}/100"
        )
        ...
        output_file = "/Users/romaingirardi/Documents/Ancient Free Will Database/kg_edge_audit_results.json"
