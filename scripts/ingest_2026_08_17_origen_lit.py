#!/usr/bin/env python3
"""Apply the 2026-08-17 Origen/patristic literature delta to the KG.

Idempotent merge of scripts/data_2026_08_17_origen_lit.json into
...
- invariants are asserted before save and backups use suffix .bak-origen_lit.

Usage: python3 scripts/ingest_2026_08_17_origen_lit.py [--apply]
Default is dry-run.
"""
...
DELTA = ROOT / "scripts/data_2026_08_17_origen_lit.json"
GATE = ROOT / "scripts/check_ingestion_rules.py"
BAK_SUFFIX = ".bak-origen_lit"
INGEST_SCRIPT = "scripts/ingest_2026_08_17_origen_lit.py"
...
def assert_delta_invariants(delta: dict) -> None:
    ...
    for node in delta["nodes"]:
        ...
        if node["type"] == "argument":
            ...
            assert source_file and source_file.endswith(".md"), (
                f"source_file must be an .md path: {node['id']}"
            )
    ...
    for edge in delta["edges"]:
        ...
        if edge["relation"] in DIALECTICAL_RELATIONS:
            ...
            assert attestation and re.search(r"\b(?:p{1,2}\.|pages?)\s*\d", attestation), (
...
def main() -> int:
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", suffix="-origen-lit.json", encoding="utf-8", delete=False
        ) as handle:
            json.dump(
