"""Apply Amand B8 consolidation (Intro §I + Lucien + Oinomaos + Néoplatoniciens).

Idempotent orchestrator:
- Updates 13 existing nodes (metadata enrichments)
- Inserts new nodes (5 persons + 8 works + 2 concepts + 15 syntheses + 4 arguments = 34)
- Inserts edges (authorship, discussion, containment, evidenced_by, influences,
  precedes, critiques, responds_to, interprets)
...
Usage:
    cd [local-path]
    .venv/bin/python3 scripts/apply_amand_b8_consolidation.py
"""
...
from amand_b8_data import UPDATES
from amand_b8_edges import NEW_EDGES
from amand_b8_inserts import NEW_INSERTS
from amand_b8_utils import (
    EDGES_PATH,
    NODES_PATH,
    dump_jsonl,
    dump_metadata,
...
def main() -> int:
    ...
    print(f"\n=== B8 EDGES ({len(NEW_EDGES)} candidates) ===")
