"""Apply Fürst 2022 B1 consolidation.

Idempotent orchestrator for batch B1 of Alfons Fürst, *Wege zur Freiheit.
Menschliche Selbstbestimmung von Homer bis Origenes* (Mohr Siebeck 2022,
Tria Corda 15).

- Updates existing nodes (metadata enrichments for the scholar/publication
  pair, Plato, Aristotle, Chrysippus, Epicurus, Carneades, Epictetus, Cicero,
  Alcinous, Calcidius, Plutarch, Alexander, Philo, Justin, Clement, Origen +
  Origen's De Principiis + De Oratione + Philocalia + Contra Celsum + Comm.
  Rom. ; concept enrichments for autexousion, prohairesis, eph hemin, liberum
  arbitrium ; scholar enrichments for Dihle, Frede, Bobzien, Kobusch, Crouzel,
  Sharples, Karamanolis, List, Kahn, Kane).
- Inserts new nodes (1 person/scholar: Hengstermann ; 5 works: Origen Hom.
  Jér., Comm. Cant., Comm. Jn, Comm. Mt., Comm. Gen. ; 3 concepts: Selbstbestimmung
  modern, Freiheitsmetaphysik origenian, kompatibilistischer Libertarismus ;
  15 scholarly arguments ; 11 syntheses chapter-by-chapter).
- Inserts edges (authorship by Fürst, syntheses+arguments as part_of the
  publication, scholarly discussions, influence chains Carneades→Origen /
  Philo→Origen / Alcinous→Origen / Alexander→Origen / Justin→Origen /
  Clement→Origen / Origen→Plotinus + Origen→Gregory Nyssa, critiques of
  Dihle / Karamanolis, support of Bobzien / Hengstermann / Kobusch,
  cites_primary_source for all major Origen works).
...
Usage:
    cd [local-path]
    .venv/bin/python3 scripts/apply_furst_2022_b1_consolidation.py
"""
...
from furst_2022_b1_data import UPDATES
from furst_2022_b1_edges import NEW_EDGES
from furst_2022_b1_inserts import (
    NEW_ARGUMENTS,
    NEW_CONCEPTS,
    NEW_PERSONS,
    NEW_SYNTHESES,
...
from furst_2022_b1_utils import (
    EDGES_PATH,
    NODES_PATH,
    dump_jsonl,
    dump_metadata,
...
NEW_INSERTS = NEW_PERSONS + NEW_WORKS + NEW_CONCEPTS + NEW_SYNTHESES + NEW_ARGUMENTS
...
def main() -> int:
    ...
    print(f"\n=== Fürst 2022 B1 UPDATES ({len(UPDATES)} candidates) ===")
    upd_applied, upd_skipped = apply_updates(nodes_by_id, UPDATES)
    print(f"  -> {upd_applied} applied, {upd_skipped} skipped (missing)")

    print(f"\n=== Fürst 2022 B1 INSERTS ({len(NEW_INSERTS)} candidates) ===")
    print(
        f"  Breakdown : {len(NEW_PERSONS)} persons + {len(NEW_WORKS)} works + "
        f"{len(NEW_CONCEPTS)} concepts + {len(NEW_SYNTHESES)} syntheses + "
        f"{len(NEW_ARGUMENTS)} arguments"
    )
    ...
    print(f"\n=== Fürst 2022 B1 EDGES ({len(NEW_EDGES)} candidates) ===")
