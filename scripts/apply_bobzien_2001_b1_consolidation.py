"""Apply Bobzien 2001 consolidation batch B1 (Determinism and Freedom in Stoic Philosophy).

Idempotent orchestrator for batch B1:
- Updates existing nodes (scholar bio + publication overhaul + 25+ ancient
  persons / works / arguments / concepts metadata enrichments)
- Inserts new nodes (4 concepts + 8 chapter syntheses + 15 scholarly arguments)
- Inserts edges (authorship, discusses, interprets, critiques, cites_primary_source,
  influence chains, internal cross-links)

Targets : the 8 chapters of Bobzien 2001 — Determinism and Fate, Two
Chrysippean Arguments, Modality, Divination, Idle Argument, Compatibilism,
Epictetus + eph_hemin/eleutheria, PHILOPATOR + rise/fall of freedom problem.
...
Usage:
    ...
    .venv/bin/python3 scripts/apply_bobzien_2001_b1_consolidation.py
"""
...
from bobzien_2001_b1_data import UPDATES
from bobzien_2001_b1_edges import NEW_EDGES
from bobzien_2001_b1_inserts import (
    NEW_ARGUMENTS,
    NEW_CONCEPTS,
    NEW_PERSONS,
    NEW_SYNTHESES,
...
def main() -> int:
    ...
    print(f"\n=== B1 EDGES ({len(NEW_EDGES)} candidates) ===")
