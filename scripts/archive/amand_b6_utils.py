"""Utilities for Amand B6 consolidation (Eusebius + Basil + Gregory Nazianzus)."""
...
def parse_metadata(node: dict[str, Any]) -> dict[str, Any]:
    """metadata is stored as JSON string in this KG; safely parse it."""
...
def dump_metadata(d: dict[str, Any]) -> str:
    """Serialise metadata back to string form expected by the KG."""
    return json.dumps(d, ensure_ascii=False)


def merge_metadata(existing: dict[str, Any], updates: dict[str, Any]) -> dict[str, Any]:
    """Shallow merge with updates taking precedence."""
...
def amand_metadata(
    *,
    ...
    """Standardised Amand-claim metadata block."""
    md: dict[str, Any] = {
        "amand_location": {
