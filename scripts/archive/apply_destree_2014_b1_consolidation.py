"""Apply Destrée/Salles/Zingano 2014 B1 consolidation.

Idempotent orchestrator for batch B1 of the Destrée 2014 edited volume:
- Inserts 1 publication node (the volume itself)
- Inserts new persons (16: 15 scholars + 1 editorial group)
- Inserts new works (3: EE, Porphyry Peri tou eph' hêmin, Proclus De Providentia)
- Inserts new concepts (2: two-sidedness, causal conception Salles)
- Inserts 23 syntheses (1 intro + 22 chapter syntheses)
- Inserts 22 arguments (one per chapter)
- Inserts metadata UPDATES on ~40 existing nodes (scholars + ancient persons +
  ancient works + central concepts)
- Inserts ~200+ edges (authorship, syntheses, discusses, interprets, critiques,
  cites_primary_source, influences, part_of, etc.)
...
Usage:
    cd [local-path]
    .venv/bin/python3 scripts/apply_destree_2014_b1_consolidation.py
"""
...
from destree_2014_b1_data import UPDATES
from destree_2014_b1_edges import NEW_EDGES
from destree_2014_b1_inserts import (
    NEW_ARGUMENTS,
    NEW_CONCEPTS,
    NEW_PERSONS,
    NEW_PUBLICATION,
...
def main() -> int:
    ...
    print(f"\n=== Destrée 2014 B1 UPDATES ({len(UPDATES)} candidates) ===")
    upd_applied, upd_skipped = apply_updates(nodes_by_id, UPDATES)
    print(f"  -> {upd_applied} applied, {upd_skipped} skipped (missing)")

    print(f"\n=== Destrée 2014 B1 INSERTS ({len(NEW_INSERTS)} candidates) ===")
    print(
        f"  Breakdown : {len(NEW_PUBLICATION)} publication + "
        f"{len(NEW_PERSONS)} persons + {len(NEW_WORKS)} works + "
    ...
    print(f"\n=== Destrée 2014 B1 EDGES ({len(NEW_EDGES)} candidates) ===")
