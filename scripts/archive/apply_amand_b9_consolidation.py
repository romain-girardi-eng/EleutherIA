"""Apply Amand B9 consolidation (Livre II patristiques secondaires).

Idempotent orchestrator for batch B9 :
- Updates existing nodes (metadata enrichments for Justin, Tatien, Irenee,
  Bardesane, Clement and his 3 works, Methode and his 2 works + 1 arg)
- Inserts new nodes (5 persons + 8 works + 1 concept + 14 syntheses + 11 arguments)
- Inserts edges (authorship, syntheses, evidenced_by Clement & Irenaeus,
  cites_primary_source, influence chains, precedes, critiques, member_of)

Targets : Justin Ch. I + Tatien note, Irenee Ch. II + Hippolyte note,
Bardesane Ch. III, Clement Ch. IV, Methode Ch. VI, Epiphane Ch. X,
Diodore de Tarse Ch. XI, Commentateur arien de Job Ch. XIII.
...
Usage:
    cd [local-path]
    .venv/bin/python3 scripts/apply_amand_b9_consolidation.py
"""
...
from amand_b9_data import UPDATES
from amand_b9_edges import NEW_EDGES
from amand_b9_inserts import (
    NEW_ARGUMENTS,
    NEW_CONCEPTS,
    NEW_PERSONS,
    NEW_SYNTHESES,
...
def main() -> int:
    ...
    print(f"\n=== B9 EDGES ({len(NEW_EDGES)} candidates) ===")
