...
The raw table cannot be read as four error rates. In the SC stratum, all 40 authorities were the same files used for ingestion; zero substantive differences therefore demonstrate extraction fidelity, not independent editorial correctness. The TLG stratum mixed two populations. Seventeen *Magna Moralia* text re-ingestions produced no substantive difference after inspection, whereas 20 of 23 sampled Plotinus records were mechanically classified as substantive even though their median CER was only 1.1%. Human inspection identified edge truncation and Perseus/TLG edition variants rather than corruption of the text body. In the Perseus/web stratum, 19 sources were locally unavailable; the remaining raw differences broadly reflect inter-edition variance, with one outlier requiring item-level review. In the First1KGreek stratum, modern curated extracts were initially compared with larger TLG citation blocks, and span misalignment dominated the distance measure; a second, span-aligned collation pass was therefore run on every substantive record of the two web strata.

The span-aligned re-collation (49 records re-measured on their best-aligned authority span, with boundary truncation counted separately from mid-span divergence) resolves the two inconclusive strata. In the First1KGreek stratum, 33 of 40 sampled records remain substantive after alignment — including all 24 Sextus Empiricus records, which show internal lexical omissions rather than boundary artefacts — while 7 fall to minor edition variance; the paradigmatic misalignment case (Epicurus, *SV* 56) aligns at 0 differing letters over 57. In the Perseus/web stratum, 8 of 12 remain substantive on the aligned span (median intra-span CER 1.51%). The corrected corpus-level statement is: 41 of 160 sampled passages (25.6%) show substantive divergence from an independent authority; 41 of 141 (29.1%) among the collatable subset; 21.9% as a weighted lower bound under an explicit convention for the 19 unavailable sources. The operational consequence is a targeted re-ingestion queue (the 24 Sextus passages from their declared First1KGreek XML witnesses, with a tokenised completeness guard) rather than a wholesale re-ingestion.
